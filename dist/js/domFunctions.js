export const setPlaceholderText = () => {
  const input = document.getElementById("search-bar-text");
...
const updateWeatherLocationHeader = (message) => {
  const h1 = document.getElementById("current-forecast-location");
...
const clearDisplay = () => {
  ...
  const sixDayForecast = document.getElementById("daily-forecast-contents");
...
const setFocusOnSearch = () => {
  document.getElementById("search-bar-text").focus();
};
...
const createElem = (elemType, divClassName, text, unit) => {
...
const displayCurrentConditions = (currentConditionsArr) => {
  const ccContainer = document.getElementById("current-forecast-conditions");
  currentConditionsArr.forEach((cc) => {
...
const displayDailyForecast = (dfArray) => {
  const dayDiv = createElem("div", "forecast-day");
  dfArray.forEach((el) => {
    dayDiv.appendChild(el);
  });
  const dailyForecastContainer = document.getElementById(
    "daily-forecast-contents"
  );
